...
function processConfig() {
  if (!localStorage.getItem('browser-vision')) {
    const defaultConfig = {images: [], allImages: [], task: 'Tell me what you think about this image'};
    localStorage.setItem('browser-vision', JSON.stringify(defaultConfig));
  }
  return JSON.parse(localStorage.getItem('browser-vision'));
}

function setUpTask(config) {
  const taskEl = document.getElementById('browser-vision-task');
  taskEl.addEventListener('keyup', changeTask.bind(this, config));
  taskEl.value = config.task;
}

function changeTask(config, event) {
  config.task = event.target.value;
  localStorage.setItem('browser-vision', JSON.stringify(config));
}
...
function setUpCapture(config) {
  ...
  captureButton.addEventListener('click', captureScreenshot.bind(this, config, gallery));
}
...
function setUpRequest(config) {
  ...
  sendButton.addEventListener('click', sendRequest.bind(this, config));
}
